...
int tintin_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *tintin_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "tintin");
        gb_context_set_save_id(interpreted_ctx, "tintin");
        if (serial_stdout) {
            generated_ctx->callbacks.on_serial_byte = gbrt_serial_stdout;
        }
        tintin_init(generated_ctx);
        tintin_init(interpreted_ctx);
        if (data_mod_file) {
    ...
    gb_context_set_save_id(ctx, "tintin");
