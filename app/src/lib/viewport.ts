...
export function readScalingMode(): ScalingMode {
  try {
    return localStorage.getItem("display-scaling") === "fit" ? "fit" : "integer";
  } catch {
    return "integer";
  }
}
...
export function frameViewport(width: number, height: number, sourceWidth: number,
  sourceHeight: number, mode: ScalingMode) {
